...
const AssetAttributePage: React.FC = () => {
  ...
  const [selectedAttribute, setSelectedAttribute] = useState<AssetAttribute | null>(null);
  ...
  const handleDelete = (record: AssetAttribute) => {
    Modal.confirm({
      ...
      onOk: async () => {
        try {
          await deleteAssetAttributeByName(record.name);
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (modalMode === 'create') {
        ...
        if (values.displayName !== selectedAttribute.displayName) {
          patch.push({
            op: 'replace',
            path: '/displayName',
        ...
        if (values.description !== selectedAttribute.description) {
          patch.push({
            op: 'replace',
            path: '/description',
        ...
        if (values.attributeCategory !== selectedAttribute.attributeCategory) {
          patch.push({
            op: 'replace',
            path: '/attributeCategory',
            value: values.attributeCategory,
          });
        }
        if (values.dataType !== selectedAttribute.dataType) {
          patch.push({
            op: 'replace',
            path: '/dataType',
        ...
        if (values.required !== selectedAttribute.required) {
          patch.push({
            op: 'replace',
            path: '/required',
    ...
    },
    {
      ...
      render: (required: boolean) => (
        <Tag
          ...
          style={required ? { color: '#cf1322', background: '#fff1f0', borderColor: '#ffa39e' } : undefined}
        >
          {required
  ...
  return (
    <PageLayoutV1 className="asset-attribute-page" pageTitle={t('label.asset-attribute-management')}>
      <div className="p-lg">
        {pageHeader}
