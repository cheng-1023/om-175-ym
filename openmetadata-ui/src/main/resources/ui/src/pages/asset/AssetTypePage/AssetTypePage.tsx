...
} from '../../../assets/svg/ic-asset-types.svg';
import {
...
const AssetTypePage: React.FC = () => {
  ...
  const [attributesError, setAttributesError] = useState<AxiosError | null>(null);
  ...
  const [selectedTypeForEdit, setSelectedTypeForEdit] = useState<AssetType | null>(null);
  ...
  // 添加属性Modal状态
  const [isAddAttributeModalVisible, setIsAddAttributeModalVisible] = useState(false);
  const [availableAttributes, setAvailableAttributes] = useState<AssetAttribute[]>([]);
  const [selectedAttributeToAdd, setSelectedAttributeToAdd] = useState<string | undefined>();
  ...
  const handleDeleteType = (type: AssetType) => {
    Modal.confirm({
      ...
      onOk: async () => {
        try {
          await deleteAssetTypeByName(type.name);
  ...
  const handleAddAttribute = async () => {
    if (!selectedType || !selectedAttributeToAdd) {
    ...
    try {
      const attributeToAdd = allAttributes.find(attr => attr.id === selectedAttributeToAdd);
      if (attributeToAdd) {
        ...
        const newAttributes = [...currentAttributes, {
          ...
          displayName: attributeToAdd.displayName,
          fullyQualifiedName: attributeToAdd.fullyQualifiedName,
        }];
        ...
        setSelectedAttributeToAdd(undefined);
      }
    } catch (error) {
  ...
  const handleOpenAddAttributeModal = () => {
    ...
    // 获取可用的属性（排除已添加的属性）
    const existingAttributeIds = selectedType.attributes?.map(attr => attr.id) || [];
  ...
  const tabItems = useMemo(() => {
    ...
    return categories.map((category) => ({
      key: category,
      label: `${t(`label.attribute-category-${category}`)} (${groupedAttributes[category].length})`,
      children: (
        <Table
          columns={[
            {
    ...
    <PageLayoutV1 pageTitle={t('label.asset-type-management')}>
          ...
          children: (
            <div className="p-lg">
              {selectedType ? (
                <>
                  ...
                  {/* Tab 页 (去除了已移到头部的 Add 按钮和标题行) */}
                    ...
                    {isLoadingAttributes ? (
                      ...
                      <Tabs
                        defaultActiveKey={Object.keys(groupedAttributes)[0] || 'all'}
                        items={tabItems}
                      />
                    )}
                  </Card>
                ...
                <Card>
                  <div className="text-center p-lg">
                    <p className="text-grey-muted">{t('label.select-type-view-details')}</p>
                  </div>
                </Card>
              )}
            </div>
      ...
      >
        <Space direction="vertical" size="middle" style={{ width: '100%' }}>
          ...
          <div>
            <label className="block mb-sm font-semibold">
              {t('label.display-name')} <span className="text-red-500">*</span>
            </label>
            <Input
        ...
        <Space direction="vertical" size="large" className="w-full">
          <div>
            ...
            {availableAttributes.length === 0 && (
              <Text type="secondary" className="mt-xs block">
