...
const AssetCategoryPage: React.FC = () => {
  ...
  const handleDelete = (record: AssetCategory) => {
    Modal.confirm({
      ...
      onOk: async () => {
        try {
          await deleteAssetCategoryByName(record.name);
  ...
  const filtersBar = (
    <Row align="middle" gutter={[16, 16]} justify="space-between">
      <Col xs={24} md={18}>
        <Button icon={<PlusIcon width={16} height={16} />} type="primary" onClick={handleAdd}>
  ...
  return (
    <PageLayoutV1 className="asset-category-page" pageTitle={t('label.asset-category-management')}>
      <div className="p-lg">
        {pageHeader}
